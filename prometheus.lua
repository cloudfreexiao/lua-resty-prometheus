...
local resty_counter_lib = require("prometheus.prometheus_counter")
local key_index_lib = require("prometheus.prometheus_keys")
local shared = require("prometheus.shared")
...
local KEY_INDEX_PREFIX = "__skynet_prom__"
...
local function check_metric_and_label_names(metric_name, label_names)
    ...
    for _, label_name in ipairs(label_names or {}) do
        ...
        if not label_name:match("^[a-zA-Z_][a-zA-Z0-9_]*$") then
            return "Metric '" .. metric_name .. "' label name '" .. label_name .. "' is invalid"
        end
    end
end
...
local function lookup_or_create(self, label_values)
    ...
    if cnt ~= self.label_count or (self.label_count > 0 and not label_values[1]) then
        return nil, string.format("inconsistent labels count, expected %d, got %d", self.label_count, cnt)
    end
    local t = self.lookup
...
local ERR_MSG_COUNTER_NOT_INITIALIZED =
    "counter not initialized! " ..
    "Have you called Prometheus:init() from the " .. "init_worker_by_lua_block nginx phase?"
...
local function del(self, label_values)
    ...
    if self.typ ~= TYPE_GAUGE then
        self:log_error("waiting ", self.parent.sync_interval, "s for counter to sync")
        skynet.sleep(self.parent.sync_interval * 100)
    end
...
function Prometheus.init(conf)
    ...
    if self.dict == nil then
        error(
            "Dictionary '" ..
                dict_name .. "' does not seem to exist. " .. "Please define the dictionary using `lua_shared_dict`.",
            2
        )
    end
...
function Prometheus:init_worker(sync_interval)
    if self._counter then
        self:log_error(
            "init_worker() has been called twice. " ..
...
local function register(self, name, help, label_names, buckets, typ)
    ...
    local name_maybe_historgram = name:gsub("_bucket$", ""):gsub("_count$", ""):gsub("_sum$", "")
    if
        (typ ~= TYPE_HISTOGRAM and (self.registry[name] or self.registry[name_maybe_historgram])) or
