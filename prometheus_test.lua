...
Nginx.timer = {}
function Nginx.timer.every(_, _, _)
end
...
function TestPrometheus:testInitOptions()
    ...
    local p4 =
        require("prometheus").init(
        "metrics",
        {
            prefix = "foo",
            sync_interval = 3,
            error_metric_name = "foobar"
        }
    )
...
function TestPrometheus:testCustomBucketer2()
    local hist3 = self.p:histogram("l3", "Histogram 3", {"var"}, {0.000005, 5, 50000})
